...
export default function OtpPage() {
  ...
  // Ref levé dès qu'on a un OTP valide — bloque le guard de mount
  // pendant la fenêtre entre clearPendingEmail() et router.replace()
  const isNavigating = useRef(false);

  // ── Guard : récupère l'email pending, redirige si absent ────────────────────
  useEffect(() => {
    if (isNavigating.current) return; // On est en train de partir → ne pas interférer
  ...
  const submitOtp = async (code: string) => {
    ...
    try {
      ...
      if (data?.token && data?.user) {
        ...
        // ORDRE CRITIQUE :
        // 1. Lever le drapeau EN PREMIER — le guard useEffect est neutralisé
        isNavigating.current = true;
        // 2. Nettoyer le pending email
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="h-1 w-full bg-gradient rounded-2xl" />
