 ...
 *   2. Saisie du code OTP à 6 chiffres (auto-submit au 6ème chiffre)
 *   3. authService.verifyOtp(email, code) → POST /{prefix}/verify-otp
 *      Le préfixe a été stocké par authService.login() en step 1.
 *   4. Succès : session persistée → redirection vers le dashboard selon le rôle
 *
 * ORDRE CRITIQUE de nettoyage (évite le race condition guard/navigation) :
 *   1. isNavigating.current = true   (neutralise le guard useEffect)
 *   2. authService.clearPendingEmail()
 *   3. router.replace(dashboardRoute)
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
export default function OtpPage() {
  ...
  const handleChange = useCallback((value: string, index: number) => {
    ...
    // Auto-submit au remplissage du dernier champ
    if (value && index === OTP_LENGTH - 1) {
